...
void Run_routine_check_up_script(void)
{
    ...
    uint16_t    vbat = 0,temp = 0,buffer_size = 0,prev_buffer_size = 0;
    ...
    if(!UART2_ReceiveBufferIsEmpty())
    {
        ...
        buffer_size = UART2_CONFIG_RX_BYTEQ_LENGTH - buffer_size;     
        if(buffer_size > 0)
        {
            while(buffer_size != prev_buffer_size)
            {
            ...
            UART2_ReadBuffer((uint8_t *)buf, buffer_size);

            // Check for URC of MQTT service failure.
            ptr = Quectel_data_from_response(buf,"QMTPING");
            ...
            {
                param = chartointeger(ptr);
                if(param == gprs_comm_interface.mqtt_config_info.client_idx)
                {
                    // Client index matched.move to next parameter.
                    ptr += 2;
                    param = chartointeger(ptr);
                    if(param == 1)
                    {
                        // PING test fail. MQTT connection closed by the client.
