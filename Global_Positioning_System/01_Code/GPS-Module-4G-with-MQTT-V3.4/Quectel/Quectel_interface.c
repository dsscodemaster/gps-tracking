...
bool Quectel_power_up(void)
{
    ...
    Quectel_send_command_and_check_response("",NULL,buf,128,5,false);   /* TODO -> Need to enhance time later */ 
    ...
    if(strstr(buf,"RDY"))
    {
        if(strstr(buf,"+CFUN: 1"))
        {
            ...
            Quectel_send_command_and_check_response("AT+QSIMSTAT=0\r\n","OK\r\n",NULL,48,1,true);    /* Disable URC */
...
//            }
    ...
    // TESTING.
    Quectel_send_command_and_check_response("AT\r\n",NULL,buf,48,2,true);
    test++;
...
void Quectel_initialize_interface(void)
{
    // Set Reset modem hardware PIN configuration.
    QUECTEL_RESET_WRITE = LOW;
    QUECTEL_RESET_TRIS = OUTPUT;
    
    // Set Interrupt-on-change(IOC) functionality for metal sensor input pin.
