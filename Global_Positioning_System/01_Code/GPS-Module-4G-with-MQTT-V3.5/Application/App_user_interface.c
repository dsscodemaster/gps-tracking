...
void App_user_interface_setup(void)
{
    ...
    // Calculate minimum voltage level for warning.
    temp = (QUECTEL_OPERATING_VOLTAGE_MAX - QUECTEL_OPERATING_VOLTAGE_MIN);
    temp = ((temp * 20) / 100);     /* 20% of full scale range */
    Quectel_common_interface.battery_volt_warning_level = QUECTEL_OPERATING_VOLTAGE_MIN + temp;
}
