...
bool App_quectel_common_get_local_timestamp(char *timestamp)
{
    if(Quectel_common_interface.bits.gsm_nw_up != true)
        goto TIME_GET_FAIL;    
    
    //--------------------------------------------------------------------------
    char buf[64] = {'\0'},*ptr,*ptr1;
    uint16_t a[8];
    
    if(!Quectel_send_command_and_check_response("AT+CCLK?\r\n","OK\r\n",buf,48,2,true))
        goto TIME_GET_FAIL;
    
    buf[strlen(buf)-4] = '\0';
    ptr = strstr(buf,"CCLK");
    if(!ptr) goto TIME_GET_FAIL;
    
    ptr += 7;
    ptr1 = strstr(buf,"\"\r\n");
    if(!ptr1)   goto TIME_GET_FAIL;

    *ptr1 = '\0';
    sscanf(ptr,"%02d/%02d/%02d,%02d:%02d:%02d",&a[2],&a[1],&a[0],&a[3],&a[4],&a[5]);
    if((a[0]>0) &&(a[0]<32) && (a[1]>0) && (a[1]<13) && (a[2]>18) && (a[2]<99))
    {
        if(a[3] >= 24)
            a[3] -= 24;
        
        sprintf(timestamp,"%02d/%02d/%02d,%02d-%02d-%02d",a[0],a[1],a[2],a[3],a[4],a[5]);
        return true;
    }
...
void Run_routine_check_up_script(void)
{
    ...
    {
        // Send command.
        Quectel_send_command_and_check_response("AT+QADC=0\r\n","OK\r\n",buf,48,1,true);
        ptr = Quectel_data_from_response(buf,"QADC");
        ...
        {
            // *ptr indicates ADC port that currently read.
            ptr1 = strstr(ptr,",");
            ptr1 += 1;
            // *ptr1 indicates voltage of the connected ADC port. here battery
            // voltage is connected with the ADC port.
            ptr2 = strstr(ptr1,"\r\n");
            ptr2 = '\0';
            vbat = atoi(ptr1);
