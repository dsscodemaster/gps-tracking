...
bool data_comm_MQTT_publish(char *data, uint8_t client_index, char *publish_topic, uint8_t packet_length)
{
    char command_buff[128],resp_buff[64],*ptr;
    uint8_t timeout = 0,client_idx = 10,result = 10;    
    
    // Prepare command.
    sprintf(command_buff,"AT+QMTPUBEX=%d,1,2,0,\"%s\",%d\r\n",client_index,publish_topic,packet_length);                                                         
    
    if(Quectel_send_command_and_check_response(command_buff,">",resp_buff,25,10,true))
    {
        ...
        Quectel_send_command_and_check_response(data,NULL,resp_buff,26,timeout,true);
        ...
        if(ptr != NULL)
        {
            ...
            {
                // Received response is not for this client.
                gprs_comm_interface.mqtt_config_info.last_pub_msg_status = CLIENT_INDEX_MISMATCH;
                return false;
            }
        }
        else
...
{
    double lat,log,speed;

    if(gps_common_interface.core_parameters.speed == 0 || gps_common_interface.core_parameters.latitude == 0 ||
       gps_common_interface.core_parameters.longitude == 0)
    {
    ...
    speed = (gps_common_interface.core_parameters.speed * KNOT_TO_KMPH_MUL_FACTOR);
    speed /= 1000;
    
    lat = (gps_common_interface.core_parameters.latitude / ((double)1000000));
    log = (gps_common_interface.core_parameters.longitude / ((double)1000000));
    
    //sprintf(buffer,"%s,%.6lf,%.6lf,%.3lf",gprs_comm_interface.datastats.macid,lat,log,speed);
...
void Send_GPS_location_to_server(void)
{
    ...
    if(send_gps_info)
    {
        ...
        {
            if(prepare_data_buffer_to_send(gps_common_interface.data_packet_buffer))
            {
                // Update status.
                gps_common_interface.bits.location_data_loaded = true;
                len = strlen(gps_common_interface.data_packet_buffer);
                
                // Run MQTT data packet publish service.
                data_comm_MQTT_publish(gps_common_interface.data_packet_buffer,gprs_comm_interface.mqtt_config_info.client_idx,
