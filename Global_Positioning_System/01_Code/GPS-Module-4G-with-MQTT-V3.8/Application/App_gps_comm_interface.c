        ...
        if(Get_gnss_state())
        {
               ...
               _badchecksumhandler != true)
            {
                ...
                if(!_isValid)
                {
                    ...
                    gps_common_interface.core_parameters.latitude = 0;
                    gps_common_interface.core_parameters.longitude = 0;                
                }
                else
                {
                    // Get all accuracy parameters.
                    gps_common_interface.core_parameters.fixQuality = getFixQuality();
                    gps_common_interface.core_parameters.numSattelites = getNumSatellites();
                    gps_common_interface.core_parameters.hdop = getHDOP();
                    
                    if(gps_common_interface.core_parameters.fixQuality < 4)
                    {
                        if((gps_common_interface.core_parameters.hdop >= 13 ||                   /* HDOP should not be greater than 1.3 */
                           gps_common_interface.core_parameters.numSattelites < 4) &&
                           gps_common_interface.accuracy_fail_counter < HDOP_THRESHOLD_FAIL_COUNTER)
                        {
                           ...
                           gps_common_interface.accuracy_fail_counter++;
                                ...
                                gps_common_interface.core_parameters.hdop > 20)
                        {
                           get_data = false;
                           gps_common_interface.accuracy_fail_counter++;                            
                        }
                    ...
                    if(get_data)
                    {
                        gps_common_interface.core_parameters.latitude = getLatitude();
                        gps_common_interface.core_parameters.longitude = getLongitude();
            ...
            *ptr = NULL, c= NULL, len = 0, counter = 0;
            ...
            {
                // NMEA sentence is void.
                gps_common_interface.core_parameters.speed = 0;
            }
            else if(*ptr1 == 'A') /* A = Active */
            {
                ...
                {
                    // Get parameter.
                    gps_common_interface.core_parameters.speed = getSpeed();                
                }
            }
            else
            {
