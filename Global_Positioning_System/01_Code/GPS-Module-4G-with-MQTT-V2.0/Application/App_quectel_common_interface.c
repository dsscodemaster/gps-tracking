...
bool App_quectel_common_perform_network_registration(void)
{
...
//        return false; 
    ...
    if((strlen(Quectel_common_interface.svc_provider) == 0))    // && (strlen(app_settings.svr_config.apn) == 0))
    {
        ...
        Quectel_send_command_and_check_response("AT+COPS?\r\n","OK\r\n",buf,48,5,true);
        ptr = Quectel_data_from_response(buf,"COPS");
        if(ptr == NULL) 
        {
        ...
        else
        {
            if((ptr1 = strstr(ptr,",\"")) != NULL)
            {
                ...
                {
                    *ptr2 = '\0';
                    sprintf(Quectel_common_interface.svc_provider,"%s",ptr1+2);
                }
            }
        }
    }
