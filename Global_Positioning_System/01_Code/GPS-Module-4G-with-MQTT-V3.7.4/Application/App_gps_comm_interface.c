...
bool Turn_on_gnss(void)
{
    ...
    // Send command.
    if(Quectel_send_command_and_check_response("AT+QGPS=1,255,10,0,1\r\n","OK\r\n",NULL,0,5,true))
        return true;
        ...
        if(Get_gnss_state())
        {
               ...
               _badchecksumhandler != true)
            {
                ...
                else
                {
                    ...
                    {
                        if((gps_common_interface.hdop >= 11 ||                   /* HDOP should not be greater than 1.1 */
                    ...
                    if(get_data)
                    {
                        ...
                        if(getAltitude(&alt))
                            gps_common_interface.altitude = alt;
