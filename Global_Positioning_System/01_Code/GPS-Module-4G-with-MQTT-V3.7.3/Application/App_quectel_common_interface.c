...
void Run_routine_check_up_script(void)
{
    ...
    if(check_battery_voltage)
    {
        ...
        else
        {
            ...
            // Update data.
            Quectel_common_interface.battery_remaining_volt_percentage = vbat_percentage;
            
            if(Quectel_common_interface.battery_remaining_volt_percentage < 20)
            {
               LOW_BATTERY_INDICATION_WRITE = HIGH; 
               
               // Check lower limit of battery percentage to operate.
               if(Quectel_common_interface.battery_remaining_volt_percentage < 8)
               {
            ...
            else if(Quectel_common_interface.battery_remaining_volt_percentage >= 21)
